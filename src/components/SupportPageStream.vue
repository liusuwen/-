...
const enterType= ref('enter');

// 默认支持 SSE 协议
async function startSSE(text?: string) {
  try {
    suggestedMessage.value = [];
    messages.value.push({
      key: messages.value.length + 1,
      role: 'user',
      content:text ?? senderValue.value,
      placement: 'end',
      loading: false,
      shape: 'corner',
      variant: 'outlined',
    ...
    payload.value.query = text ?? senderValue.value;
    ...
    const response = await fetch('http://192.168.3.140/v1/chat-messages', {
      headers: {
        'Authorization': `Bearer app-mp1zOEFWQ8ICIsGQ4ZepYpp9`,
...
const suggestMessage= async (messageId:string) => {
  if (suggestedMessage.value.length==0 && messageId!=resourceMessageId.value){
    if (!data.value.length)
      return '';
    if (messageId!==''){
      const response = await getSuggestedMessage(messageId);
      suggestedMessage.value = response.data;
      resourceMessageId.value = messageId;
      loading.value = false;
      handleCancel();
    }
  }
  ...
  for (let index = 0; index < data.value.length; index++) {
    ...
    try {
      ...
      if (parse.event === 'message_end') {
        ...
        suggestMessage(JSON.parse(chunk).message_id);
      }
    } catch (error) {
...
<!--            <img :src="item.role === 'ai' ?-->
<!--            avartAi : avatar" alt="avatar">-->
<!--            <img v-if="item.role==='ai'" src="@/assets/support.png" alt="avatar">-->
            <div v-if="item.role==='ai'" class="avatar-wrapper">
              <img src="@/assets/support.png" alt="avatar">
            </div>
